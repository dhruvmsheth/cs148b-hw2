...
using on-policy rollouts from the current policy weights at each step:

\begin{table}[h]
\centering
\caption{GRPO validation accuracy at each evaluation checkpoint (50 steps total).}
\label{tab:grpo}
\begin{tabular}{lcccccc}
\toprule
Method & Baseline & Step 9 & Step 19 & Step 29 & Step 39 & Step 49 \\
\midrule
GRPO (\texttt{normalize\_by\_std=True}) & 0.219 & 0.457 & 0.637 & 0.703 & 0.734 & 0.727 \\
\bottomrule
\end{tabular}
\end{table}

The model improves rapidly, reaching 0.70+ accuracy by step 29 and peaking
at 0.734 at step 39.
The mean rollout reward also increases steadily from under 10\% in the first
few steps to 50--70\% by step 40--47, confirming the policy is genuinely learning.
The slight drop at step 49 (0.727 vs 0.734) is normal variance given 256 eval
examples.
