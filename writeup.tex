...
\midrule
GRPO (\texttt{normalize\_by\_std=True})  & 0.2266 & 0.2266 \\
GRPO (\texttt{normalize\_by\_std=False}) & 0.2344 & 0.2344 \\
\bottomrule
\end{tabular}
\end{table}

Both variants improve slightly over the CoT baseline (0.2188) after 50 steps.
The no-normalize variant outperforms std normalization because at this scale
most groups have low reward variance (0 or 1 correct out of 8 rollouts),
causing the std denominator to squash the gradient signal.

The mean training reward stays around 0.03--0.12 per batch since the base
model generates with temperature 1.0 and only occasionally produces correct
answers. Longer training (500+ steps) with on-policy weight syncing would
show more pronounced improvement, as the generation policy would gradually
produce more correct rollouts, increasing the effective learning signal.
